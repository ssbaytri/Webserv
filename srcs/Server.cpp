...
void Server::_handleDashboard(const Request& request, Response& response, const ServerConfig& config)
{
    ...
    // generate dashboard HTML with session data matching the dark glassmorphism theme
    std::string html =
        "<!DOCTYPE html>\n"
        "<html lang=\"en\">\n"
        "<head>\n"
        ...
        "    <style>\n"
        "        :root { --bg: #0f172a; --card: #111827; --accent: #38bdf8; --accent-glow: rgba(56, 189, 248, 0.4); --text: #e5e7eb; --muted: #9ca3af; }\n"
        "        * { box-sizing: border-box; }\n"
        "        body { margin: 0; min-height: 100vh; font-family: system-ui, -apple-system, BlinkMacSystemFont, \"Segoe UI\", sans-serif; "
        "background: linear-gradient(135deg, #020617, #0f172a, #1e1b4b); background-size: 200% 200%; animation: gradientBG 15s ease infinite; color: var(--text); display: flex; align-items: center; justify-content: center; }\n"
        "        @keyframes gradientBG { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }\n"
        "        .container { max-width: 600px; width: 100%; padding: 20px; }\n"
        "        .card { background: rgba(17, 24, 39, 0.8); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.05); border-radius: 20px; padding: 40px; text-align: center; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.4), 0 0 20px rgba(56, 189, 248, 0.05); }\n"
        "        h1 { margin: 0 0 10px 0; font-size: 2.5rem; color: #fff; text-shadow: 0 4px 20px rgba(0,0,0,0.5); }\n"
        "        h1 span { color: var(--accent); text-shadow: 0 0 15px var(--accent-glow); }\n"
        "        p.status { color: #10b981; font-weight: 500; margin-bottom: 30px; display: inline-flex; align-items: center; gap: 8px; }\n"
        "        .status::before { content: ''; width: 8px; height: 8px; background: #10b981; border-radius: 50%; box-shadow: 0 0 8px #10b981; }\n"
        "        .info-box { background: rgba(2, 6, 23, 0.6); border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 12px; padding: 20px; text-align: left; margin-bottom: 30px; }\n"
        "        .info-row { margin-bottom: 15px; }\n"
        "        .info-row:last-child { margin-bottom: 0; }\n"
        "        .info-label { color: var(--muted); font-size: 0.85rem; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 5px; display: block; }\n"
        "        .info-value { color: #fff; font-family: monospace; font-size: 1rem; word-break: break-all; }\n"
        "        .logout { display: inline-block; padding: 12px 30px; background: rgba(239, 68, 68, 0.1); color: #ef4444; text-decoration: none; border: 1px solid #ef4444; border-radius: 12px; font-weight: 600; transition: all 0.3s ease; }\n"
        "        .logout:hover { background: #ef4444; color: #fff; box-shadow: 0 0 20px rgba(239, 68, 68, 0.4); transform: translateY(-2px); }\n"
        "    </style>\n"
        "</head>\n"
        "<body>\n"
        "    <div class=\"container\">\n"
        ...
        "            <p class=\"status\">Successfully authenticated</p>\n"
        "            \n"
        "            <div class=\"info-box\">\n"
        "                <div class=\"info-row\">\n"
        ...
        "                </div>\n"
        "                <div class=\"info-row\">\n"
        "                    <span class=\"info-label\">Logged In At</span>\n"
        "                    <span class=\"info-value\">" + std::string(std::ctime(&session->createdAt)) + "</span>\n"
        "                </div>\n"
        "            </div>\n"
        "            \n"
